...
export const CompletedCalendarView: React.FC = () => {
  ...
  const handleToggleTaskStatus = async (task: Task) => {
    ...
    if (selectedTaskPopup?.id === task.id) {
      if (task.status === 'done') {
        ...
        setSelectedTaskPopup({ ...task, status: 'done' });
      }
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col relative">
      <div className="px-4 sm:px-8 pt-4 sm:pt-8 flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-zinc-900 tracking-tight">Completed Tasks</h1>
          <p className="text-sm sm:text-base text-zinc-500 mt-1 sm:mt-2">Review finished work and accomplishments.</p>
        </div>
      </div>
        ...
        {isMobile ? (
          ...
          >
            {getTasksForDay(currentDate).length === 0 ? (
              <div className="text-center text-zinc-500 mt-10">No completed tasks for this day.</div>
            ) : (
                ...
                return (
                  <div 
                    ...
                    className={clsx(
                      ...
                      "opacity-70 line-through bg-emerald-50/30"
                    )}
                  >
                  ...
                  <div>
                    ...
                    <div className="p-6">
                      <div className="flex justify-between items-start mb-4">
                        <h3 className="text-xl font-bold text-zinc-900 pr-8 line-through opacity-80">{task.title}</h3>
                        <button 
                          onClick={() => setSelectedTaskPopup(null)}
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          ...
                          {task.completed_at && (
                            <div className="flex items-center gap-2 text-emerald-600 col-span-2">
                              ...
                              <span>Completed on {format(parseISO(task.completed_at), 'MMM d, yyyy h:mm a')}</span>
                            </div>
                          )}
                        </div>
                      </div>
